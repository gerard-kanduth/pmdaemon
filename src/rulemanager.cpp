...
void RuleManager::loadRules() {
    ...
    for(auto& file: fs::directory_iterator(rules_dir)) {
        if (regex_match(file.path().u8string(), regex_rule_file)) generateRuleFromFile(string(file.path()));
    }
}
...
bool RuleManager::registerRule(unordered_map<string, string> file_content) {
    ...
    try {
        ...
        if (!file_content["REGEX_SEARCH_PATTERN"].empty()) {
            rule.regex_search_pattern_string = file_content["REGEX_SEARCH_PATTERN"];
            rule.regex_search_pattern = std::regex(rule.regex_search_pattern_string);
        }
        ...
        (file_content["SEND_NOTIFICATIONS"] == "0") ? rule.send_notifications = false : rule.send_notifications = true;
        ...
        if (rule.regex_search_enabled && rule.regex_search_pattern_string.empty()) {
        ...
        if (!rule.regex_search_enabled && !rule.regex_search_pattern_string.empty() && rule.command.empty()) {
        ...
        if (logger->getLogLevel() >= DEBUG1) {
            ...
            if (rule.regex_search_enabled) {
                logger->logDebug("regex_search_pattern: " +rule.regex_search_pattern_string);
            } else {
    ...
    if (!rules_file) {
        ...
        if (rules_file.is_open()) {
            ...
            while(getline(rules_file, line)) {
                if (available_rule_settings.find(line.substr(0, line.find("="))) != available_rule_settings.end()) rule.insert(pair<string,string>(line.substr(0, line.find("=")),line.substr(line.find("=")+1)));
            }
            rules_file.close();
        }
...
void RuleManager::showRuleContent(Rule rule) {
    ...
    logger->logInfo("regex_search_pattern: " + rule.regex_search_pattern_string);
...
void RuleManager::showRules() {
    for (auto& r : rules) showRuleContent(r.second);
}
